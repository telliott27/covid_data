...
gpdf %>% 
  ...
  geom_area(fill = ghColorSelect("blue"), alpha = 0.8) +
...
gpdf <- state_county %>% 
  ...
  ungroup()
gpdf %>% 
  ggplot(aes(x = date, y = cases_per_capita, group = county, label = end_label, color = core)) +
  geom_line(aes(size = linesize, alpha = linesize)) +
...
gpdf <- state_county %>% 
  ...
  ungroup()
gpdf %>% 
  ggplot(aes(x = date, y = daily_new_per_capita, group = county, label = end_label, color = core)) +
  geom_line(aes(size = linesize, alpha = linesize)) +
  geom_text_repel(segment.color = NA, nudge_x = 2) +
  scale_y_continuous(labels = scales::comma) +
  theme_covid +
...
nyt_county_daily %>% 
  ...
  group_by(date) %>% 
  summarize(
    daily_deaths_raw = sum(deaths),
    daily_deaths = sum(deaths_avg),
    .groups = "drop"
  ) %>% 
  mutate(
    daily_deaths_raw = ifelse(daily_deaths_raw < 0, NA, daily_deaths_raw)
  ) %>% 
  ggplot(aes(x = date, y = daily_deaths_raw)) +
  geom_area(fill = ghColorSelect("purple"), alpha = 0.8) +
...
gpdf <- state_county %>% 
  ...
  ungroup()
gpdf %>% 
  ggplot(aes(x = date, y = deaths_per_capita, group = county, label = end_label, color = core)) +
  geom_line(aes(size = linesize, alpha = linesize)) +
  geom_text_repel(segment.color = NA, nudge_x = 2) +
  scale_y_continuous(labels = scales::comma) +
  theme_covid +                                                                                  
...
gpdf <- state_county %>% 
  ...
  ungroup()
gpdf %>% 
  ggplot(aes(x = date, y = daily_deaths_per_capita, group = county, label = end_label, color = core)) +
  geom_line(aes(size = linesize, alpha = linesize)) +
  geom_text_repel(segment.color = NA, nudge_x = 2) +
  scale_y_continuous(labels = scales::comma) +
  theme_covid +
  labs(title = str_glue("Daily New COVID Deaths Per Capita in Bay Area Counties"),
       x = NULL,
       y = "New Deaths per 100,000 Population")
```
