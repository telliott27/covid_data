...
nyt_county_data %>% 
  ...
  group_by(county) %>% 
  filter(date == max(date)) %>% 
  select(date, county, cases, deaths, population, weekly_change)
```
...
gpdf <- state_df %>% 
  ...
  mutate(
    days_since = date - min(date),
    core = if_else(state %nin% top_n_state(state_df, 7, wt = percent_change), "", state),
    end_label = if_else(date == max(date), core, NA_character_),
    linesize = if_else(core == "", 0.5, 1)
  ) %>% 
  filter(!is.na(change_per_capita))
gpdf %>% 
  ...
  ggplot(aes(x = date, y = percent_change, group = state, color = core,size = linesize, label = end_label)) +
  geom_line() +
  geom_text_repel(segment.color = NA, nudge_x = 2, size = 4) +
  theme_covid +
  labs(
...
ggsave("img/usa_state_percent_change.png", width = 8, height = 6, dpi = 400)
```
