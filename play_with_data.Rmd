...
gpdf <- nyt_county_data %>% 
  filter(full_name %in% top_n_counties(nyt_county_data, n = 20, wt = cases))
gpdf <- gpdf %>% 
  group_by(county) %>% 
  mutate(days_since = date - min(date),
         core = if_else(
...
gpdf %>% 
  ...
  scale_size_identity() +
  guides(color = FALSE) +
  labs(title = "Cumulative Cases of Coronavirus in U.S. Counties",
       x = "Days Since First Case",
       y = "Cases")
ggsave("img/counties_raw.png", width = 8, height = 6, dpi = 400)
```

```{r}
gpdf <- nyt_county_data %>% 
  filter(full_name %in% top_n_counties(nyt_county_data, n = 20, wt = deaths))
gpdf <- gpdf %>% 
  group_by(county) %>% 
  mutate(days_since = date - min(date),
         core = if_else(
           full_name %nin% top_n_counties(gpdf, n=7, wt = deaths), "", 
           full_name 
           ),
         linesize = if_else(core == "", 0.5, 1),
         end_label = if_else(date == max(date), core, NA_character_)) %>% 
...
gpdf %>% 
    ggplot(aes(x = days_since, y = deaths, group = full_name, label = end_label, color = core)) +
  geom_line(aes(size = linesize)) +
  geom_text_repel(segment.color = NA, nudge_x = 2) +
  #scale_y_log10() +
  #scale_x_continuous(limits = c(0,100)) +
  ...
  scale_size_identity() +
  guides(color = FALSE) +
  labs(title = "Cumulative Deaths of Coronavirus in U.S. Counties",
       x = "Days Since First Case",
       y = "Deaths")
ggsave("img/deaths_counties_raw.png", width = 8, height = 6, dpi = 400)
```
...
```{r}
start_date <- floor_date(today() - months(2), unit = "month")
end_date <- today() + days(14)
gpdf %>% 
  filter(date >= start_date) %>% 
